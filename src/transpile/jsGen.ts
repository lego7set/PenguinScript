...
export default class JSGenerator {
  ...
  public transpile(type: "string" | "func" | "generator", yields?: boolean, warpTimer?: boolean, isWarp?: boolean): string | TranspiledFunction | TranspiledGenerator {
    ...
    this.src = `try{${this.src}}catch(error){if(error?.isExit)return error?.returnValue;if(error instanceof Error){error.name = "(PenguinScript Evaluation) " + error.type};throw error};`
    switch (type) {
      case "string": {
        return this.src;
      }
